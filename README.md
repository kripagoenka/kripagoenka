# Hi, I'm Kripa Goenka 👋

🎓 B.Tech CSE @ MIT World Peace University, Pune (2023–2027)
🏆 Smart India Hackathon 2025 — Top 50 Teams
💻 Interested in QA Automation, AI Tools & Backend Systems
📍 Pune, India | kripagoenka1@gmail.com

## 🛠️ Skills
- **Languages:** C, C++, Python, JavaScript, SQL
- **Testing:** Manual Testing, API Testing (Postman), Regression Testing
- **Tools:** Git, GitHub, CI/CD, VS Code
- **Frontend:** HTML, CSS, React.js
- **Learning:** Linux, pytest, OpenAI API

## 🚀 Currently Building
- Python API Test Automation Framework
- AI-powered Bug Classifier using OpenAI API

## 📫 Let's connect
- Email: kripagoenka1@gmail.com
